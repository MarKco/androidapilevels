 ...
 */
class APILevelsContent {

    companion object: APILevels {

        /**
         * An array of SDK version.
         */
        val apiLevelsList: MutableList<SingleAPILevel> = ArrayList()

        /**
         * A map of SKD Versions items, by ID.
         */
        val apiLevelsListByCode: MutableMap<String, SingleAPILevel> = HashMap()
        ...
        init {
            ...
            val items =
                arrayListOf(
                    ...
                    SingleAPILevel("Ice Cream Sandwich", "4.0 - 4.0.4", "October 18, 2011", false, 14, 15),
        ...
        override fun getAPILevels(): MutableList<SingleAPILevel> {
            return apiLevelsList;
        }

        private fun addItems(arrayList: ArrayList<SingleAPILevel>) {
            for (singleItem in arrayList) {
                addItem(singleItem)
            }
        }

        private fun addItem(item: SingleAPILevel) {
            apiLevelsList.add(item)
            apiLevelsListByCode.put(item.codeName, item)
        }
    }
}